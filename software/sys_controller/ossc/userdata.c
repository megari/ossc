...
int export_userdata()
{
    ...
    const alt_u8 prompt_ofs[] = { 0, 16, 31, LNG(48, 44), };
    const char *prompt_msgs =
        ...
        "ｺﾞﾁｭｳｲｸﾀﾞｻｲ!!!"    "\0" // [16..30]
        "1=ﾂﾂﾞｸ 2=ﾔﾒﾙ"     "\0" // [31..43]
        "ﾄﾞﾁﾗｶｴﾗﾝﾃﾞｸﾀﾞｻｲ"        // [44..59]
        );
    alt_u32 btn_vec;
...
out:
    ...
    switch (retval) {
        case 0:
            msg = LNG("Success", "kanryou");
            break;
        case SD_NOINIT:
            msg = LNG("No SD card det.", "SDkaado?");
            break;
        case -EINVAL:
            msg = LNG("Invalid params.", "parameetaaeraa");
            break;
        case UDATA_EXPT_CANCELLED:
            msg = LNG("Cancelled", "kyanseru");
            break;
        default:
            msg = LNG("SD/Flash error", "SD/FLASHeraa");
            break;
    }
    strncpy(menu_row2, msg, LCD_ROW_LEN+1);
